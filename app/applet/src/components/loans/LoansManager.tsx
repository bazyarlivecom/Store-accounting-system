...
export default function LoansManager({
  ...
  // New state for fetching data for balance calculation
  ...
  useEffect(() => {
    ...
    getIssuedChecks().then(setIssuedChecks);
    getReceivedChecks().then(setReceivedChecks);
  ...
  const handleCreateLoan = () => {
    if (!formData.personId || formData.amount === '' || formData.totalInstallments === '' || formData.installmentAmount === '') {
      alert('لطفا تمام فیلدهای ضروری را پر کنید.');
      return;
    }
    if (!useBalanceAsAmount && !formData.accountId) {
      alert('لطفا حساب بانکی / صندوق را انتخاب کنید.');
      return;
    }
    ...
    // If we use balance, we perform an adjustment to clear balance.
    let txnType = '';
    let category = '';
    let desc = '';
    
    if (useBalanceAsAmount) {
       txnType = formData.type === 'given' ? 'receive' : 'pay';
       category = 'loan_balance_adjustment';
       desc = `تبدیل مانده حساب شخص به وام ${formData.type === 'given' ? 'پرداختی' : 'دریافتی'}`;
    } else {
       txnType = formData.type === 'given' ? 'pay' : 'receive';
       category = formData.type === 'given' ? 'loan_given' : 'loan_received';
       desc = `ثبت اولیه وام ${formData.type === 'given' ? 'پرداختی' : 'دریافتی'}`;
    }
    ...
    const newTransaction = {
      ...
      type: txnType,
      amount: amountNum,
      accountId: useBalanceAsAmount ? (accounts[0]?.id || '') : formData.accountId, // Ensure an account is set if needed for data constraints, though adjustment shouldn't practically need one
      personId: formData.personId,
      categoryId: category,
      description: desc,
    ...
    const newLoansArray = [...loans, newLoan];
    const newInstArray = [...installments, ...newInstallments];
    
    setLoans(newLoansArray);
    setInstallments(newInstArray);
    
    import('../../services/dataService').then(({ saveLoans, saveInstallments }) => {
      saveLoans(newLoansArray);
      saveInstallments(newInstArray);
      addTransaction(newTransaction as any).then(() => {
  ...
  const selectedPersonBalance = formData.personId ? calculatePersonBalance(formData.personId) : null;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <div className="col-span-1 border border-gray-200 rounded-2xl p-1 bg-gray-50/50 flex">
                <button
                   ...
                   className={`flex-1 py-3 text-sm font-black rounded-xl transition-all ${formData.type === 'given' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'} disabled:opacity-50`}
                >پرداختی به شخص</button>
                <button
                   disabled={useBalanceAsAmount}
                   onClick={() => setFormData({...formData, type: 'received'})}
                   className={`flex-1 py-3 text-sm font-black rounded-xl transition-all ${formData.type === 'received' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'} disabled:opacity-50`}
             ...
             <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                     ...
                     const instAmt = val && formData.amount ? Math.round(Number(formData.amount) / val) : '';
                    ...
                    <div className="p-6 flex flex-col lg:flex-row items-center gap-6 cursor-pointer" onClick={() => setExpandedLoanId(isExpanded ? null : loan.id)}>
                       ...
                       <div className="flex-1 w-full flex flex-col md:flex-row md:items-center justify-between gap-6">
                         ...
                         <div>
                            ...
                            {loan.description && (
                               <p className="text-xs text-gray-400 mt-2 line-clamp-1">{loan.description}</p>
                            )}
                         </div>
